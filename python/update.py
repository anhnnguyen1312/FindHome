...
def call_api():
    ...
    # Kết nối vào cơ sở dữ liệu bằng mysql
        ...
        # print(df_text1)
        df_text2 = pd.read_sql_query("SELECT * FROM users ", conn)
        print(df_text2)
        dfLike = pd.read_sql_query('SELECT u."userId", STRING_AGG(l."postId"::TEXT, ',' ORDER BY l."postId") AS "postIds" FROM "userLikes" l JOIN (SELECT DISTINCT "userId" FROM "userLikes") u ON u."userId" = l."userId" GROUP BY u."userId";', engine)
        df = pd.read_sql_query(query, conn)
